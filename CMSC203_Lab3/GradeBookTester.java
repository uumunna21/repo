//package package1;
//
//import static org.junit.jupiter.api.Assertions.*;
//
//import org.junit.jupiter.api.AfterEach;
//import org.junit.jupiter.api.BeforeEach;
//import org.junit.jupiter.api.Test;
//
//class GradeBookTester {
//
//	@BeforeEach
//	void setUp() throws Exception {
//	}
//
//	@AfterEach
//	void tearDown() throws Exception {
//	}
//
//	@Test
//	void testAddScore() {
//		fail("Not yet implemented");
//	}
//
//	@Test
//	void testSum() {
//		fail("Not yet implemented");
//	}
//
//	@Test
//	void testMinimum() {
//		fail("Not yet implemented");
//	}
//
//	@Test
//	void testFinalScore() {
//		fail("Not yet implemented");
//	}
//
//	@Test
//	void testGetScoreSize() {
//		fail("Not yet implemented");
//	}
//
//	@Test
//	void testToString() {
//		fail("Not yet implemented");
//	}
//
//}

package package1;

import static org.junit.jupiter.api.Assertions.*;

import org.junit.jupiter.api.AfterEach;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;

class GradeBookTester {

    private GradeBook gradeBook1;
    private GradeBook gradeBook2;

    @BeforeEach
    void setUp() throws Exception {
        // Create two GradeBook objects
        gradeBook1 = new GradeBook(5);
        gradeBook2 = new GradeBook(5);

        // Add scores to gradeBook1
        gradeBook1.addScore(85.0);
        gradeBook1.addScore(90.0);

        // Add scores to gradeBook2
        gradeBook2.addScore(78.0);
        gradeBook2.addScore(88.5);
    }

    @AfterEach
    void tearDown() throws Exception {
        // Set the GradeBook objects to null
        gradeBook1 = null;
        gradeBook2 = null;
    }

    @Test
    void testAddScore() {
    	// Add scores to the GradeBook
        assertTrue(gradeBook.addScore(85.5));  // Check if score is added successfully
        assertTrue(gradeBook.addScore(92.0));
        assertTrue(gradeBook.addScore(78.5));

        // Check the string representation of the scores in the GradeBook
        assertEquals("85.5 92.0 78.5", gradeBook.toString());

        // Check the number of scores added
        assertEquals(3, gradeBook.getScoreSize());
    }

    @Test
    void testSum() {
    	// Add scores to the GradeBook
        gradeBook.addScore(85.5);
        gradeBook.addScore(92.0);
        gradeBook.addScore(78.5);

        // Check if the sum of the scores is correct
        assertEquals(256.0, gradeBook.sum(), 0.001);
    }

    @Test
    void testMinimum() {
    	// Add scores to the GradeBook
        gradeBook.addScore(85.5);
        gradeBook.addScore(92.0);
        gradeBook.addScore(78.5);

        // Check if the minimum score is correct
        assertEquals(78.5, gradeBook.minimum(), 0.001);
    }

    @Test
    void testFinalScore() {
    	// Test case with no scores
        assertEquals(0, gradeBook.finalScore());

        // Add scores to the GradeBook
        gradeBook.addScore(85.5);
        assertEquals(85.5, gradeBook.finalScore());  // Only one score

        gradeBook.addScore(92.0);
        gradeBook.addScore(78.5);

        // Check if the final score (sum with the lowest score dropped) is correct
        assertEquals(177.5, gradeBook.finalScore(), 0.001);
    }

    @Test
    void testGetScoreSize() {
        fail("Not yet implemented");
    }

    @Test
    void testToString() {
        fail("Not yet implemented");
    }
}

